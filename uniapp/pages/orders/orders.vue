		...
		<view class="content" :style="{ paddingTop: navbarHeight + 'px' }">
			...
			<view class="order-list">
					...
					>
						<view class="order-header">
							<view class="order-title-box">
								...
								<view class="order-status" :class="order.pickedUp ? 'completed' : order.isStolen ? 'stolen' : 'active'">
				...
				<view v-if="activeTab === 'pickup'">
					<view
						class="order-card"
						:class="{ active: !order.pickedUp && !order.isStolen }"
						v-for="order in pickupOrders"
						:key="order.id"
						v-memo="[order.id, order.pickedUp, order.isStolen]"
						@click="viewOrderDetail(order)"
					>
						<view class="order-header">
							<view class="order-title-box">
						...
						<view class="order-info">
							...
							<view class="info-row" v-if="order.recipientInfo && order.recipientInfo.remark">
								<text class="info-label">备注</text>
								<text class="info-value">{{ order.recipientInfo.remark }}</text>
							</view>
						</view>
					</view>
					...
					<view v-if="pickupOrders.length === 0" class="empty-state">
						<image class="empty-icon" src="/static/icons/icon-pickup.png" mode="aspectFit" lazy-load></image>
						<text class="empty-text">暂无待取件订单</text>
						<button class="empty-btn" @click="goToPickup">去取件</button>
					</view>
				</view>
			</view>
		</view>
...
const pickupOrders = computed(() => {
	// 显示待取件订单（type为store但picked_up为false的订单，即用户作为收件人的订单）
	return pendingOrders.value.filter(order => order.type === 'store' && !order.pickedUp)
})
...
const loadOrders = async () => {
	try {
		const serverOrders = await cabinetApi.getOrders()
		...
		orders.value = serverOrders.map(order => {
			const formatted = {
				...
				isStolen: order.is_stolen || false,
...
const loadPendingOrders = async () => {
	try {
		...
		const pendingOrderList = await cabinetApi.getPendingOrders(userInfo.phone)

		// 转换服务器返回的数据格式
		pendingOrders.value = pendingOrderList.map(order => {
			const formatted = {
				id: order.id,
				type: order.type,
				cabinetName: order.cabinet_name,
...
const viewOrderDetail = (order) => {
	const isPickupTab = activeTab.value === 'pickup'
	uni.showModal({
		title: isPickupTab ? '待取件详情' : '存件详情',
		content: `
智能柜: ${order.cabinetName || '-'}
格子号: ${order.cellNumber || '-'}号
取件码: ${order.pickupCode || '-'}
存件时间: ${formatTime(order.createTime)}
状态: ${isPickupTab ? '待取件' : (order.pickedUp ? '已取出' : '在柜中')}
${order.recipientInfo && order.recipientInfo.remark ? '备注: ' + order.recipientInfo.remark : ''}
		`.trim(),
		showCancel: false
	})
}
